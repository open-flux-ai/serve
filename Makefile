...
build-llama-7b-gpu:
	flux pull artifact --timeout=120m0s \
		--output ./build \
		oci://ghcr.io/llm-gitops/models/llama-2-7b-chat-4k:v1.0.0-q5km-gguf
...
build-llama-70b-gpu:
	flux pull artifact --timeout=120m0s \
		 --output ./build \
		oci://ghcr.io/llm-gitops/models/llama-2-70b-chat-4k:v1.0.0-q5km-gguf
	mv ./build/*.gguf ./build/model.gguf
	cp llama-cpp-python/Dockerfile.cublas-bundle build/Dockerfile
